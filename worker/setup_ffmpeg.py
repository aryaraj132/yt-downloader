...
Ensures FFmpeg is available before starting the worker.

Copied from legacy backend/setup_ffmpeg.py with minor adjustments.
"""
import os
import sys
import shutil
...
def verify_ffmpeg():
    ...
    Checks local bin/ first, then system PATH (for Docker where ffmpeg is apt-installed).
    ...
    # Fallback: check system PATH (Docker installs via apt)
    try:
        import subprocess
